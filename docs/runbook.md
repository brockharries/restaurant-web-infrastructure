...
- **Origin box unrecoverable:** stand up the stack on a fresh box from version-controlled compose + config, restore the database from the latest off-box dump, restore assets from the latest snapshot. This is the path the **tested restore** exists to make fast. The timed test tells me how long this really takes, which is the number to hold up against the recovery time the business can live with.
...
- **Edge outage:** static/cached pages may still serve from the CDN. If the whole edge is down, the fallback is a direct-to-origin DNS change (kept documented), accepting temporarily reduced protection to keep orders flowing. Because the origin firewall normally restricts 443 to the edge provider's IP ranges, this failover also means temporarily opening 443 beyond those ranges, and re-tightening it once traffic is back behind the edge. This only works if a few things are in place before the outage. The origin needs a publicly trusted certificate for the hostname, because an origin certificate issued by the edge provider is trusted by the edge and not by browsers, so customers going direct would get a certificate warning. I need a way to change DNS that still works while the edge is down, which is not a given when the same provider hosts the DNS. And the firewall change needs a rollback I have actually run. Until those have been tested together, I would not call this a proven fallback.
